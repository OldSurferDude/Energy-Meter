...
float cumulativeEnergy = 0;   // will need to initialize this somehow.
//------------------------------------------------------loop
void loop() {
  ...
  if (millis()-averagingStart > AVERAGING_PERIOD){
    ...
    float voltage = ADC_COUNTS_TO_VOLTAGE * VrmsADC / float(averagingCount);
    float current = ADC_COUNTS_TO_CURRENT * IrmsADC / float(averagingCount);
    float VA = voltage * current;
    float PowerFactor = Watts/VA;
    send(msgPOWER.set(Watts,2));
    wait(MESSAGEWAIT);
    send(msgVOLTAGE.set(voltage,1));
    wait(MESSAGEWAIT);
    send(msgCURRENT.set(current,1));
    wait(MESSAGEWAIT);
    send(msgVA.set(VA,2));
    wait(MESSAGEWAIT);
    send(msgPOWER_FACTOR.set(PowerFactor,2));
    wait(MESSAGEWAIT);
    
    cumulativeEnergy += (Watts * AVERAGING_PERIOD / 1000.0) * (1/(1000*3600)); // Watt-seconds converted to kWh
    send(msgENERGY.set(cumulativeEnergy,1));
